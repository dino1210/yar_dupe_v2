...
export default function ToolsAndEquipmentsLogsTable() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/tools-logs")
      ...
      .catch((error) => {
        console.error("Error fetching tools and equipments logs", error);
        setLoading(false);
      });
  }, []);
            ...
            {currentConsumables.length > 0 ? (
              ...
              <TableRow>
                <TableCell className="px-5 py-4 text-start text-red-500 dark:text-red-400">
                  Error Fetching Tools and Equipments Logs
                </TableCell>
              </TableRow>
            )}
          </TableBody>
